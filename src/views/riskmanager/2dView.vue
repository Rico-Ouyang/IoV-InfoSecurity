...
<script>
import gSearch from '@/components/D3print/gSearch.vue'
import d3graph from '@/components/D3print/d3graph.vue'
export default {
  name: '2DView',
  components: {
    gSearch,
  ...
  methods: {
    ...
    d3jsonParser (json) {
      ...
      //   }
      // }
      for (let transitions of json) {
        for (item of transitions){
          // 重新更改data格式
          if (nodeSet.indexOf(item.seller) == -1) {
            nodeSet.push(item.seller)
            nodes.push({
              id: item.seller,
              label: 'seller',
              properties: item.seller
            })
          }
          if (nodeSet.indexOf(item.buyer) == -1) {
            nodeSet.push(item.buyer)
            nodes.push({
              id: item.buyer,
              label: 'buyer',
              properties: {
                house: item.objectOfSale
              }
            })
          }
          links.push({
